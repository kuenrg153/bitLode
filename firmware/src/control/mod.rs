...
impl Controller {
    pub async fn run(&mut self) {
        loop {
            ...
            if self.reboot_to_bootloader {
                // Let the host collect the ack, then enter the ROM USB bootloader (both interfaces enabled).
                info!("Control: rebooting to USB bootloader");
...
pub async fn usb_task(class: CdcAcmClass<'static, super::UsbDriver>, i2c: super::I2cDriver, gpio: gpio::Pins<'static>, adc: adc::Pins<'static>, led: led::Led<'static>) -> ! {
    ...
    let mut controller = Controller { tx, i2c, gpio, adc, led, reboot_to_bootloader: false };
