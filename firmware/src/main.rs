...
async fn main(spawner: Spawner) {
    // 96 MHz sys_clk: 12 MHz xtal × 64 / (4 × 2) = 96 MHz.
    // PIO UART divider = 96 MHz / (8 × 12 MHz) = 1 exactly → zero jitter at 12 Mbaud.
    // Also works for 363,636 baud: 96 MHz / (8 × 363,636) = 33 (error < 0.001%).
    let mut clk_cfg = ClockConfig::crystal(12_000_000);
    if let Some(xosc) = clk_cfg.xosc.as_mut() {
        xosc.sys_pll = Some(PllConfig { refdiv: 1, fbdiv: 64, post_div1: 4, post_div2: 2 });
    }
    let p = embassy_rp::init(embassy_rp::config::Config::new(clk_cfg));
    ...
    let usb_config = {
        ...
        let mut config = embassy_usb::Config::new(0xe3be, 0x40e1);
    ...
    let asic_uart = {
        ...
        crate::pio_uart::PioUart::new(115200, &mut common, sm0, sm1, p.PIN_8, p.PIN_9)
    };
